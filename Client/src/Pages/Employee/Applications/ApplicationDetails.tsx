...
import { ArrowLeft, Mail, Calendar, DollarSign, FileText, Download, Briefcase, Gift, FileCheck, UserPlus, Loader2 } from 'lucide-react';
...
const ApplicationDetails = () => {
  ...
  useEffect(() => {
    if (application && application.statusName === 'Selected' && isHR) {
      checkDocumentsAndOffers();
    }
  }, [application, isHR]);
...
const checkDocumentsAndOffers = async () => {
  ...
  try {
    ...
    const candidateDocs = await documentService.getCandidateDocuments(application.candidateId);
    
    // Check if all REQUIRED documents are VERIFIED (statusId === 28)
    const requiredDocs = candidateDocs.filter(doc => doc.isRequired);
    const allVerified = requiredDocs.length > 0 && 
                        requiredDocs.every(doc => doc.statusId === 28);
    
    setHasRequiredDocs(allVerified);

    // Check if offer already exists
    try {
      const offer = await offerService.getOfferByApplicationId(application.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Sidebar */}
        <div className="space-y-6">
          {/* HR Actions Card - Only for Selected applications and HR users */}
          {isHR && application.statusName === 'Selected' && (
              ...
              <CardContent className="space-y-3">
                ...
                {!existingOffer ? (
                  <div className="bg-white rounded-lg p-3">
                    <p className="text-sm text-gray-600 mb-2">
                      {hasRequiredDocs 
                        ...
                        : 'Verify documents before creating offer'
                      }
                    </p>
                    <Button
                      className="w-full bg-green-600 hover:bg-green-700"
                      onClick={() => navigate(`/employee/offers/create/${application.id}`)}
                      disabled={!hasRequiredDocs || checkingDocs}
                    >
                  ...
                  <div className="bg-white rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge className={
                        existingOffer.statusName === 'Accepted' 
                          ? 'bg-green-100 text-green-800'
                          : existingOffer.statusName === 'Pending'
                          ? 'bg-amber-100 text-amber-800'
                          : 'bg-red-100 text-red-800'
                      }>
                        {existingOffer.statusName}
                      </Badge>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full"
                      onClick={() => navigate(`/employee/offers/${existingOffer.id}`)}
                    >
                      <Gift className="h-4 w-4 mr-2" />
                      View Offer
                    </Button>

                    {/* Onboard Button if offer is accepted */}
                    {existingOffer.statusName === 'Accepted' && (
                      <Button
                ...
                <div className="bg-white rounded-lg p-3">
                  <p className="text-xs text-gray-500 text-center">
                    Complete workflow: Documents → Offer → Onboard
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button 
                className="w-full"
                onClick={() => navigate(`/employee/interviews/schedule/${application.id}`)}
              >
                Schedule Interview
              </Button>
            </CardContent>
